...
`;
    ...
    res.status(500).json({ error: "Failed to generate AI caption", details: error.message });
  }
});
...
5. "engagementTriggers": An array of 2-3 fan interaction ideas.`;
    ...
    console.error("Error running multi-stage prompt chain:", error);
    res.status(500).json({ error: "Failed to execute multi-stage prompt chain", details: error.message });
  }
});
...
`;
    ...
    console.error("Error generating journey step:", error);
    res.status(500).json({ error: "Failed to generate journey automation step", details: error.message });
  }
});
...
app.post("/api/generate/image", async (req, res) => {
  try {
    ...
    if (leonardoKey && leonardoKey !== "MOCK_KEY" && leonardoKey.trim() !== "") {
      ...
      if (generationId) {
        // Poll for generation completion (up to 12 attempts, 2 seconds apart)
        let imageUrl = null;
...
`;
    ...
    res.status(500).json({ error: "Failed to query RAG database", details: error.message });
  }
});
...
`;
    ...
    console.error("Error summarizing RAG snippets:", error);
    res.status(500).json({ error: "Failed to generate RAG summary", details: error.message });
  }
});
...
`;
    ...
    res.status(500).json({ error: "Failed to auto-tag document", details: error.message });
  }
});
...
`;
    ...
    res.status(500).json({ error: "Failed to generate document summary", details: error.message });
  }
});
...
Format the response strictly as a JSON object matching the requested schema.`;
    ...
    console.warn("[NEWS API] Daily news digest generation failed, using high-fidelity simulated backup:", error.message || error);
    const fallback = getSimulatedDailyDigest(lang);
    res.json(fallback);
  }
});
